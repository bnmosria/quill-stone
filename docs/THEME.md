...
`Radius.*`, etc. — never a theme-specific key like `Light.Background.Base`. This keeps them
...
`App.axaml` loads the shared tokens and the default theme statically so all
`{DynamicResource}` bindings resolve on the first render frame:
...
2. Create `Styles/Themes/{Name}/Light.axaml` and `Dark.axaml` — each includes `Palette.axaml`
   via `MergedDictionaries` then maps its colors to the semantic `Brush.*` tokens.
...
`Tokens.axaml` and `ControlStyles.axaml` need no changes — they work with any theme that
provides the `Brush.*` contract.
